...
WITH p AS (
  SELECT 
    max_by(concat(val, array(batchid)), batchid) val,
    min(batchid) batchid
  FROM (
    SELECT
      split(value, "[,]") val,
...
SELECT
  ...
  if(
    isnotnull(
      if(try_cast(val[21] as int) > 1000000 or try_cast(val[12] as BIGINT) > 200000,"HighValue+","") || 
      if(try_cast(val[14] as int) > 3 or try_cast(val[20] as int) > 5,"Expenses+","") ||
      if(try_cast(val[16] as int) > 45, "Boomer+", "") ||
      if(try_cast(val[12] as BIGINT) < 50000 or try_cast(val[17] as int) < 600 or try_cast(val[21] as int) < 100000, "MoneyAlert+","") ||
      if(try_cast(val[13] as int) > 3 or try_cast(val[20] as int) > 7, "Spender+","") ||
      if(try_cast(val[16] as int) < 25 and try_cast(val[21] as int) > 1000000, "Inherited+","")
    ),
    left(
      if(try_cast(val[21] as int) > 1000000 or try_cast(val[12] as BIGINT) > 200000,"HighValue+","") || 
      if(try_cast(val[14] as int) > 3 or try_cast(val[20] as int) > 5,"Expenses+","") ||
