...
with cust as (
  SELECT 
    ...
    addressline2,
    postalcode
  FROM {wh_db}_{scale_factor}.DimCustomer
  WHERE iscurrent
)
SELECT 
  agencyid,
  ...
  networth,
  marketingnameplate
FROM {wh_db}_{scale_factor}_stage.ProspectIncremental p
JOIN {wh_db}_{scale_factor}.BatchDate recdate
  ON p.recordbatchid = recdate.batchid
JOIN {wh_db}_{scale_factor}.BatchDate origdate
