...
with Watches as (
  SELECT 
    ...
    min(batchid) batchid
  FROM (
    SELECT * FROM {wh_db}_{scale_factor}_stage.v_WatchHistory
    UNION ALL
    SELECT * FROM {wh_db}_{scale_factor}_stage.v_WatchIncremental) wh
  GROUP BY 
    w_c_id,
    w_s_symb
)
select
  ...
  wh.batchid 
from Watches wh
JOIN {wh_db}_{scale_factor}.DimSecurity s 
  ON 
    s.symbol = wh.symbol
    AND wh.dateplaced >= s.effectivedate 
    AND wh.dateplaced < s.enddate
JOIN {wh_db}_{scale_factor}.DimCustomer c 
  ON
