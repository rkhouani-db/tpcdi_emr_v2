...
WITH Customers as (
  ...
  FROM
    {wh_db}_{scale_factor}_stage.v_CustomerIncremental c
  JOIN {wh_db}_{scale_factor}.BatchDate bd 
...
JOIN {wh_db}_{scale_factor}.TaxRate r_lcl 
  ON c.lcl_tx_id = r_lcl.TX_ID
JOIN {wh_db}_{scale_factor}.TaxRate r_nat 
  ON c.nat_tx_id = r_nat.TX_ID
LEFT JOIN {wh_db}_{scale_factor}_stage.ProspectIncremental p 
  on 
