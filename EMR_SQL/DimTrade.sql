...
WITH TradeIncremental AS (
  ...
  FROM
    {wh_db}_{scale_factor}_stage.v_TradeIncremental t
  group by
    tradeid
),
TradeIncrementalHistory AS (
  ...
  SELECT
    ...
    status
  FROM
    {wh_db}_{scale_factor}_stage.v_TradeHistory
),
Current_Trades as (
  SELECT
    tradeid,
...
Trades_Final (
  SELECT
    ...
    1 batchid
  FROM 
    {wh_db}_{scale_factor}_stage.v_Trade t
    JOIN 
      Current_Trades ct
